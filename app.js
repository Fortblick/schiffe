...
function startLocalCountdown() {
  if (localCountdownTimer) return;
  localCountdownTimer = setInterval(updateCountdownDisplay, 250);
}
...
function updateCountdownDisplay() {
  if (!serverState) { stopLocalCountdown(); return; }

  // Runde-Countdown
  const roundEl = document.querySelector('[data-countdown="round"]');
  if (roundEl && serverState.phase === 'playing' && !serverState.paused) {
    const t = getRoundTimeLeft();
    roundEl.textContent = fmtTime(t);
    if (t < 10000) roundEl.classList.add('urgent');
    else roundEl.classList.remove('urgent');
  }

  // Pause-Countdown
  const pauseEl = document.querySelector('[data-countdown="pause"]');
  if (pauseEl && serverState.paused) {
    pauseEl.textContent = fmtTime(getPauseTimeLeft());
  }

  // Wenn Runde abgelaufen, kommt nächstes State-Update vom Server automatisch.
}
...
function updateLocalCountdownState() {
  // Countdown-Timer nur laufen lassen wenn er gebraucht wird
  if (serverState && (serverState.phase === 'playing' || serverState.paused)) {
    startLocalCountdown();
  } else {
    stopLocalCountdown();
  }
